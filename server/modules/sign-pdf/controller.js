import fs from "fs";
import signPdfService from "./service.js";

/**
 * Controller handling PDF signing requests.
 */
export const handleSignPdfController = async (req, res) => {
  try {
    const resultFileName = await signPdfService.signPdf(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "SIGNPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [sign-pdf]:", error);
