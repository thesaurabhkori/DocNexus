import fs from "fs";
import mergePdfService from "./service.js";

/**
 * Controller handling PDF merge operations.
 */
export const handleMergePdfController = async (req, res) => {
  try {
    const resultFileName = await mergePdfService.mergePdf(req.files);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "MERGEPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [merge-pdf]:", error);
