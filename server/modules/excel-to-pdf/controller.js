import fs from "fs";
import excelToPdfService from "./service.js";

/**
 * Controller handling Excel to PDF conversion requests.
 */
export const handleExcelToPdfController = async (req, res) => {
  try {
    const resultFileName = await excelToPdfService.convertExcelToPdf(req.files);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "EXCELTOPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [excel-to-pdf]:", error);
