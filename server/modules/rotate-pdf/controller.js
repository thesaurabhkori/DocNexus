import fs from "fs";
import rotatePdfService from "./service.js";

/**
 * Controller handling PDF page rotation requests.
 */
export const handleRotatePdfController = async (req, res) => {
  try {
    const resultFileName = await rotatePdfService.rotatePdf(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "ROTATEPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [rotate-pdf]:", error);
