import fs from "fs";
import htmlToPdfService from "./service.js";

/**
 * Controller handling HTML to PDF conversion requests.
 */
export const handleHtmlToPdfController = async (req, res) => {
  try {
    const resultFileName = await htmlToPdfService.convertHtmlToPdf(req.files);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "HTMLTOPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [html-to-pdf]:", error);
