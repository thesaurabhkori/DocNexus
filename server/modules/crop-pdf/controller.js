import fs from "fs";
import cropPdfService from "./service.js";

/**
 * Controller handling PDF crop operations.
 */
export const handleCropPdfController = async (req, res) => {
  try {
    const resultFileName = await cropPdfService.cropPdf(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "CROPPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [crop-pdf]:", error);
