import fs from "fs";
import watermarkPdfService from "./service.js";

/**
 * Controller handling PDF watermark operations.
 */
export const handleWatermarkPdfController = async (req, res) => {
  try {
    const resultFileName = await watermarkPdfService.watermarkPdf(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "WATERMARKPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [watermark-pdf]:", error);
