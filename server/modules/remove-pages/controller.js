import fs from "fs";
import removePagesService from "./service.js";

/**
 * Controller handling page removal requests.
 */
export const handleRemovePagesController = async (req, res) => {
  try {
    const resultFileName = await removePagesService.removePages(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "REMOVEPAGES execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [remove-pages]:", error);
