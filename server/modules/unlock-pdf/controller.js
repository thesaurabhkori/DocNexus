import fs from "fs";
import unlockPdfService from "./service.js";

/**
 * Controller handling PDF password removal/unlocking requests.
 */
export const handleUnlockPdfController = async (req, res) => {
  try {
    const resultFileName = await unlockPdfService.unlockPdf(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "UNLOCKPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [unlock-pdf]:", error);
