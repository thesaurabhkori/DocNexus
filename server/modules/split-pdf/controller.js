import fs from "fs";
import splitPdfService from "./service.js";

/**
 * Controller handling PDF split requests.
 */
export const handleSplitPdfController = async (req, res) => {
  try {
    const resultFileName = await splitPdfService.splitPdf(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "SPLITPDF execution completed successfully.",
      fileName: resultFileName,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [split-pdf]:", error);
