import fs from "fs";
import pdfToPdfaService from "./service.js";

/**
 * Controller handling PDF to PDF/A conversion requests.
 */
export const handlePdfToPdfaController = async (req, res) => {
  try {
    const resultFileName = await pdfToPdfaService.convertPdfToPdfa(req.files, req.body);

    const dynamicBaseUrl = `${req.protocol}://${req.get("host")}`;

    return res.status(200).json({
      success: true,
      message: "PDFTOPDFA execution completed successfully.",
      fileName: resultFileName,
      pdfaUrl: `${dynamicBaseUrl}/converted/${resultFileName}`,
      pdfUrl: `${dynamicBaseUrl}/converted/${resultFileName}`
    });

  } catch (error) {
    console.error("Pipeline crash inside action context [pdf-to-pdfa]:", error);
